...
Verbatim copy of src/driver/src/alpasim_driver/trajectory_optimizer.py into
this standalone e2e-challenge container. Only numpy/scipy.optimize deps, no
alpasim-internal imports, so it ports without pulling in alpasim_driver's
heavier dependency tree. Never enabled for Transfuser in its original
managed-flow config; wired in here behind TRANSFUSER_TRAJ_OPTIMIZER_ENABLED
with a tightened max_deviation default (see driver.py and
